module frogger(
    ...
    parameter BG_RGB = 12'h0F8;         // blue background
...
always @(posedge clk or posedge reset)
    if (reset) begin
        ...
        frog_y <= FROG_Y_START;
        score <= 0;
        is_win <= 0; // Ensure is_win is reset properly
    end
    else begin
        if (refresh_tick) begin
            if (is_win == 1) 
                is_win <= 0; // Use non-blocking assignment here

            // Check collision with obstacles
            for (i = 0; i < 10; i = i + 1) begin
                if ((obstacle_X[i] - OBSTACLE_SIZE - FROG_SIZE <= frog_x) && 
                    (frog_x <= obstacle_X[i] + OBSTACLE_SIZE + FROG_SIZE) &&
                    (obstacle_Y[i] - OBSTACLE_SIZE - FROG_SIZE <= frog_y) && 
                    (frog_y <= obstacle_Y[i] + OBSTACLE_SIZE + FROG_SIZE)) begin
                    // Reset frog position on collision
                    frog_x <= FROG_X_START; // Non-blocking assignment
                    frog_y <= FROG_Y_START; // Non-blocking assignment
                end
            end

            // Move the frog based on input (down, up, left, right)
            ...
            // Check if the frog reaches the top (win condition)
            if (frog_y - FROG_SIZE <= board_Y_MIN) begin
                // Frog reached the top, record high score
                frog_x <= FROG_X_START; // Non-blocking assignment
                frog_y <= FROG_Y_START; // Non-blocking assignment
                is_win <= 1; // Non-blocking assignment for win condition
            end
            ...
            for (i = 0; i < 10; i = i + 1) begin
                if (obstacle_direction[i] == 0) begin
                    // Move left to right
                    obstacle_X[i] <= obstacle_X[i] + OBSTACLE_VELOCITIES[i]; // Non-blocking
    ...
    always @*
        if(~video_on)
            rgb = 12'h630;       // black(no value) outside display area
            ...
            rgb = 12'h630;          // black(outside board)
    endmodule